...
The operations underneath are identical. What changes is *where the call originates*.
...
Each procedure takes a long options map. [`rewrite_data_files`](https://iceberg.apache.org/docs/latest/spark-procedures/#rewrite_data_files) alone has [over 15 tunable parameters](https://iceberg.apache.org/docs/latest/spark-procedures/#options) covering file selection, parallelism, failure recovery, and Merge-on-Read behavior. The other three follow the same pattern. Most teams call them with defaults and then wonder why compaction runs slowly, fails on large tables, or never reclaims storage.
...
The [Spark Actions API](https://iceberg.apache.org/docs/latest/api/#actions) gives you programmatic access when a SQL `CALL` doesn't offer enough control. It's still Spark underneath; you just invoke it from Java or Scala. `SparkActions.get().rewriteDataFiles(table)` returns a builder where you chain filters, set options, and execute. Same operations, same engine, embeddable in custom services and CI pipelines.
...
The [Flink Maintenance Framework](https://iceberg.apache.org/docs/latest/flink-maintenance/) is newer and less known. It lets you embed maintenance directly inside a Flink streaming job through the `TableMaintenance` API. You define a pipeline with `ExpireSnapshots`, `RewriteDataFiles`, and `DeleteOrphanFiles` as streaming operators, coordinated by a `TriggerLockFactory` (JDBC-backed) so concurrent runs don't collide. This matters if you're already on Flink: you can compact files in the same job that writes them, without standing up a separate Spark cluster.
...
The query engine opens all 10,000 files, reads 10,000 Parquet footers, and evaluates 10,000 sets of column statistics. Each file-open is a network round-trip to object storage, so what should take seconds takes minutes.
...
10,000 files × 5 MB = 50 GB, all within the same partition, comfortably fits under the 100 GB limit. One group means one rewrite task, regardless of how many Spark executors are available.
...
The result: 10,000 files become ~100. The metadata swap is atomic, so readers see either all-old or all-new files, never a partial state.
...
**[Binpack](https://iceberg.apache.org/docs/latest/spark-procedures/#usage_9)** (the default) merges files to reach the target size, with no reordering. It's fast because it reads rows and writes them back without shuffling data across the cluster.
...
After binpack compaction, the analyst's full-partition scan is fast: 100 file-opens instead of 10,000. But a filtered query like `WHERE region = 'US'` still reads most files because `region = 'US'` rows are scattered across all of them.

**[Sort](https://iceberg.apache.org/docs/latest/spark-procedures/#options-for-sort-strategy)** merges files *and* physically reorders rows by the columns you specify. This clusters related data together so queries with predicates on those columns can skip entire files using min/max statistics in the metadata.
...
For large tables this is unacceptable. A 4-hour compaction that aborts on group 8 means 4 hours of compute thrown away, and the table is still uncompacted when you wake up to retry. 

To overcome this, enable [partial progress](https://iceberg.apache.org/docs/latest/spark-procedures/#options) to let each group commit independently:
...
The defaults are conservative for streaming tables and aggressive for quiet ones. A pattern that works across both: `7-day retention` with `retain_last => 10`. That's enough history for [time travel](https://iceberg.apache.org/docs/latest/spark-queries/#time-travel-queries-with-sql) and rollback debugging on quiet tables, and enough storage release on busy ones.
...
The procedure is relatively cheap compared to compaction because it only rewrites metadata and does not touch data files. Even on large production tables, running it daily is usually operationally safe.
...
Running only part of the pipeline usually leaves performance or storage problems unresolved.
...
So far we've focused on how the four maintenance operations work internally. Running them once is straightforward. Operating them reliably across hundreds of tables is the hard part, and that's where Iceberg stops helping.
