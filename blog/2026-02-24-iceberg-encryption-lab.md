...
#### Tools
We launch a custom Alpine-based container with a number of pre-installed and pre-configured command-line tools. In this lab, we use the [AWS CLI](https://github.com/aws/aws-cli), [mc](https://github.com/minio/mc), and the MinIO KMS CLI `minkms` (see the [MinIO KMS documentation](https://docs.min.io/community/minio-object-store/administration/kms.html)) as administration tools for MinIO, as well as [hexdump](https://opensource.com/article/19/8/dig-binary-files-hexdump) and [parq](https://github.com/a-poor/parq) to inspect files stored on disk.
