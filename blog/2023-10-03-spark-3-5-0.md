...
- **dropDuplicatesWithinWatermark operations:** The dropDuplicatesWithinWatermark operation allows users to drop duplicate rows from a Structured Streaming stream within a specific watermark interval. This can be useful for applications such as anomaly detection and [real-time analytics](/glossary/real-time-analytics).
...
[Apache Spark](/glossary/apache-spark) is a key component of IOMETE, a fully-managed [lakehouse](/glossary/data-lakehouse) platform. You can use IOMETE for [cloud](https://iomete.com/product/deployment), hybrid and [on-premise](https://iomete.com/blog/on-premise-case-study) environment. Spark provides the underlying processing engine for IOMETE, enabling users to perform a wide range of data processing and analytics tasks, including:
...
Overall, You can use [Spark and IOMETE](https://iomete.com/product/data-platform/platform-overview) together to get the most benefit from your data architecture by deploying it on-premises, in the cloud, or in a hybrid environment.

---

<FAQSection faqs={[
  {
    question: "What is new in Apache Spark 3.5.0?",
    answer: "Apache Spark 3.5.0 adds the general availability of the Spark Connect Scala client, new PySpark and SQL functionality, distributed training with DeepSpeed, and Structured Streaming improvements. It also introduces SQL function support for HyperLogLog approximate aggregations and Python user-defined table functions. These changes make Spark more accessible across languages and more efficient for streaming workloads. IOMETE runs Apache Spark as its underlying processing engine, so these engine improvements carry into its lakehouse platform."
  },
  {
    question: "What is Spark Connect used for?",
    answer: "Spark Connect lets applications connect to a Spark cluster remotely using a thin client, decoupling the client from the Spark driver. This makes it easier to call Spark from other programming languages and frameworks without embedding the full runtime. In Spark 3.5.0 the Scala client reached general availability alongside support for distributed training and inference. Platforms that embed Spark, including IOMETE, can use this architecture to broaden how users reach the engine."
  },
  {
    question: "How do HyperLogLog sketches help estimate distinct counts?",
    answer: "HyperLogLog sketches are a probabilistic data structure that estimates the number of distinct elements in a large dataset using a fraction of the memory an exact count would need. Spark 3.5.0 lets developers access HLL sketches directly rather than only through approx_count_distinct(), giving more control over sketch parameters and result interpretation. This suits tasks like counting unique website visitors or detecting fraud. Spark powers the query engine inside IOMETE, where these functions are available to users."
  },
  {
    question: "Can you run Apache Spark on-premises?",
    answer: "Yes, Apache Spark runs on-premises, in the cloud, or in hybrid environments, since it is an open processing engine not tied to any single provider. On-premises Spark deployments keep data inside your own infrastructure, which matters for regulated or sensitive workloads. Running Spark this way typically pairs it with a resource manager and storage layer you control. IOMETE is a self-hosted lakehouse platform built on Apache Spark, so it can run Spark workloads within your own data center."
  }
]} />
