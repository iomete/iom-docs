---
slug: /reference/data-sources/csv-files
title: CSV Files
...
- **comment:** skip lines beginning with this character. Default is disabled. Set to `#` or another character to enable
- **nullValue:** specifies a string that indicates a null value, any fields matching this string will be set as nulls
- **dateFormat:** specifies a string that indicates the date format to use when reading dates or timestamps. Custom date formats follow the formats at `java.time.format.DateTimeFormatter`. This applies to both `DateType` and `TimestampType`. By default, it is `null` which means trying to parse timestamps and dates by `java.sql.Timestamp.valueOf()` and `java.sql.Date.valueOf()`
  :::
