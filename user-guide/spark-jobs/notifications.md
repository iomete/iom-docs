...
1. Open a job from the left sidebar via **Applications** → **Job Templates** → *job name*, or **Applications** → **Streaming Jobs** → *job name*.
...
When a silence is active, the <FlexButton label="Silence"><BellSlash size={14} /></FlexButton> button shows a yellow badge with the end date, time, and remaining duration (for example, **4/28/2026, 9:07 PM (0h 14m left)**). To end the silence early, click it again and select **Remove silence** in the popover.
