...
|----------|---------|-------|-------------|
| `verboseLogging` | false | TBD | Enables verbose (INFO-level) stdout logging for job orchestrator server and worker pods. Leave disabled for reduced (ERROR-level) log volume. |
