...
You can use Docker Compose to start a local Airflow environment with the plugin. Clone the integrations repository and run the Compose project from the plugin directory:

```bash
git clone https://github.com/iomete/iomete-integrations.git
cd iomete-integrations/iomete-airflow-plugin
docker compose up --build
```

This builds a local Airflow image, installs the latest plugin package from PyPI, and starts the Airflow webserver at `http://localhost:8080`. Startup can take a few minutes.
