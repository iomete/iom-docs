...
Let's navigate to the [Lakehouse create](./virtual-lakehouses.md#creating-a-compute-cluster) page. Here, you'l find options for `Node driver` and `Node executor`, which include node type selections. You can also view the Total CPU and Memory based on the selected executor and executor count.
