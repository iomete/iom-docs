...
:::important
The compute cluster driver should be operational at all times as it acts as the endpoint for BI tools. The executors are elastic and can be scaled up or down according to the load. Learn more about [Compute Clusters](/user-guide/compute-clusters/overview).
:::
