...
5. Add each credential as an [environment variable](/resources/user-guide/spark-jobs/spark-application-config#environment-variables) whose name matches the `${VAR_NAME}` value in the JSON. Set its value from an [IOMETE Secret](/resources/user-guide/secrets#usage-in-workloads) (recommended).
