...
SELECT * FROM db.table TIMESTAMP AS OF '2021-01-01 00:00:00.000'
```

```sql
-- time travel to a specific snapshot
SELECT * FROM db.table VERSION AS OF 1234
```

:::info
Iceberg supports time travel through both Spark SQL syntax and the DataFrame API as shown above.
:::
