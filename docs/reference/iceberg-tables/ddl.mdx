---
title: "Manage, Create and Manage Iceberg Tables Using DDL"
sidebar_label: DDL
...
Iceberg will convert the column type in Spark to the corresponding Iceberg type. Please check the section of [type compatibility on creating table](https://iceberg.apache.org/docs/latest/spark-writes/#spark-type-to-iceberg-type) for details.
...
- `TBLPROPERTIES ('key'='value', ...)` to set [table configuration](https://iceberg.apache.org/docs/latest/configuration/)
...
The `PARTITIONED BY` clause supports transform expressions to create [hidden partitions](https://iceberg.apache.org/docs/latest/partitioning/).
...
Iceberg uses table properties to control table behavior. For a list of available properties, see [Table configuration](https://iceberg.apache.org/docs/latest/configuration/).
...
```sql
ALTER TABLE sample ALTER COLUMN measurement TYPE double
ALTER TABLE sample ALTER COLUMN measurement COMMENT 'unit is bytes per second'
...
[Partition transforms](https://iceberg.apache.org/docs/latest/spark-ddl/#partitioned-by) are also supported:

```sql
ALTER TABLE sample ADD PARTITION FIELD bucket(16, id)
ALTER TABLE sample ADD PARTITION FIELD truncate(4, data)
ALTER TABLE sample ADD PARTITION FIELD year(ts)
...
ALTER TABLE sample WRITE ORDERED BY category ASC, id DESC
