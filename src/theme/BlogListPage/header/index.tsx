import React from "react";
import styles from "./styles.module.scss";

interface IProps {
  baseUrl: string;
}

function Header({ baseUrl }: IProps) {
  return (
    <div className={styles.Header}>
      <div className={styles.HeaderContent}>
        <h3>The IOMETE blog</h3>
        <p>
          Welcome to our blog. Here's where we share company news, engineering updates and educational content. Don't hesitate to reach out to us by
          email{" "}
          <a href="mailto:hello@iomete.com" className="text-primary">
            hello@iomete.com
          </a>
          .
        </p>
        <p>We'd love to hear from you!</p>
      </div>
      <div className={styles.BgImg}>
        <svg width="330" height="505" viewBox="0 0 330 505" fill="none" xmlns="http://www.w3.org/2000/svg">
          <path
            opacity="0.7"
            d="M0 449.494V132.187C0 128.887 1.65006 125.917 4.45517 123.937L185.467 1.83205C192.232 -2.78812 201.638 1.83205 201.638 10.0824V327.224C201.638 330.525 199.988 333.495 197.182 335.475L16.1706 457.579C9.40536 462.2 0 457.579 0 449.329V449.494ZM34.8163 149.843V384.647C34.8163 392.732 44.2217 397.352 50.9869 392.897L162.201 317.819C165.006 316.004 166.656 312.869 166.656 309.569V74.7648C166.656 66.6795 157.251 62.0593 150.486 66.5145L39.2715 141.592C36.4664 143.407 34.8163 146.543 34.8163 149.843Z"
            fill="currentColor"
          />
          <path
            opacity="0.85"
            d="M85.3047 471.934V154.791C85.3047 151.491 86.9547 148.521 89.7599 146.541L270.772 24.2715C277.537 19.6513 286.942 24.2715 286.942 32.5218V349.664C286.942 352.964 285.292 355.934 282.487 357.914L101.475 480.184C94.71 484.804 85.3047 480.184 85.3047 471.934ZM120.121 172.447V407.251C120.121 415.336 129.526 419.957 136.292 415.501L247.506 340.424C250.311 338.608 251.961 335.473 251.961 332.173V97.2043C251.961 89.119 242.556 84.4988 235.79 88.954L124.576 164.032C121.771 165.847 120.121 168.982 120.121 172.282V172.447Z"
            fill="currentColor"
          />
          <path
            d="M178.359 494.538V177.231C178.359 173.931 180.009 170.961 182.815 168.981L363.826 46.711C370.592 42.0908 379.997 46.711 379.997 54.9613V372.103C379.997 375.403 378.347 378.374 375.542 380.354L194.53 502.623C187.765 507.243 178.359 502.623 178.359 494.373V494.538ZM213.176 195.052V429.855C213.176 437.941 222.581 442.561 229.346 438.106L340.561 363.028C343.366 361.213 345.016 358.078 345.016 354.778V119.809C345.016 111.723 335.61 107.103 328.845 111.558L217.631 186.636C214.826 188.451 213.176 191.586 213.176 194.887V195.052Z"
            fill="currentColor"
          />
        </svg>
      </div>
    </div>
  );
}

export default Header;
